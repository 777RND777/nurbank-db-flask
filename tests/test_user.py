...
def test_get_user_applications(client, user, application, n):
    response = client.get(f"/users/{user['id_']}/applications")
    assert response.status_code == 404
...
def test_get_user_pending(client, user, application, auth):
    response = client.get(f"/users/{user['id_']}/pending")
    assert response.status_code == 404
...
def test_update_user(client, user, nickname):
    ...
    response = client.put(f"/users/{user['id_']}", json=auth)
    assert response.status_code == 404
    ...
    response = client.get(f"/users/{user['id_']}", json=user)
    assert response.status_code == 200
